            ...
            <div class="row gy-3">
                ...
                {{-- <div class="col-md-6 col-lg-4">
                        ...
                        @error('duration')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                        <div class="invalid-feedback"  id="duration-error" >{{ __('messages.duration_required') }}</div>
                    </div>
                    ...
                    <div class="col-md-6 col-lg-4">
                        ...
                        <div class="d-flex align-items-center justify-content-between form-control">
                            <div class="form-check form-switch">
                                {{ html()->hidden('skip_enabled', 0) }}
                                {{ html()->checkbox('skip_enabled', old('skip_enabled', $data->skip_enabled))
                ...
                {{-- @php
                        $enableSkip = old('skip_enabled', $data->skip_enabled ?? 0);
                    @endphp --}}
                {{-- <div class="col-md-6 col-lg-4" id="skipAfterContainer" style="display: {{ old('enable', 0) == 1 ? 'block' : 'none' }}">
                ...
                <div class="col-md-6 col-lg-4">
                    ...
                    {{ html()->select(
                            ...
                            old('target_content_type', $data->target_content_type ?? []),
...
@push('after-scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Trigger initial load if old type is available and different from stored type
            // This prevents wiping out server-rendered options on a normal page load
            const storedType = @json($data->target_content_type ?? null);
            if (oldType && oldType !== storedType) {
