...
// dd($data['episodeData']);
    ...
    public function checkMoviePurchase(Request $request)
    {
        ...
        $watchedPercentage = 0;
        if ($hasPurchased) {
             // Try to get progress from active ticket first
             ...
             if ($ticket) {
                 $progress = \DB::table('watch_progress')
                    ...
                    ->first();
                 $watchedPercentage = $progress ? $progress->watched_percentage : 0;
             } else {
                 $tracking = \DB::table('movie_user_trackings')
                    ->where('user_id', $userId)
                    ->where('entertainment_id', $movieId)
                    ->first();
                 $watchedPercentage = $tracking ? $tracking->watched_percentage : 0;
             }
        }
