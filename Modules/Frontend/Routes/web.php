...
|
*/
Route::post('/check-watch-count', [FrontendController::class, 'checkWatchCount'])->name('frontend.checkWatchCount');
Route::post('/decrement-watch-count', [FrontendController::class, 'decrementWatchCount'])->name('frontend.decrementWatchCount');
Route::post('/update-watch-progress', [FrontendController::class, 'updateWatchProgress'])
    ->name('frontend.updateWatchProgress');
    
Route::post('/ppv/check-access', [PpvWatchController::class, 'checkAccess'])->name('ppv.checkAccess');
Route::post('/ppv/save-progress', [PpvWatchController::class, 'saveProgress'])->name('ppv.saveProgress');
Route::post('/ppv/consume-ticket', [PpvWatchController::class, 'consumeTicket'])->name('ppv.consumeTicket');
...
Route::get('/movie/fetch-watch-count', [MovieController::class, 'checkWatchPermission'])->name('pay-per-view.fetch-watch-count');
Route::post('movie/decrement-watch-count', [MovieController::class, 'decrementWatchCount'])
    ->name('pay-per-view.decrement-watch-count');


Route::get('/comingsoon', [MovieController::class, 'comingSoonList'])->middleware('checkModule')->name('comingsoon');
Route::get('/leavingsoon', [MovieController::class, 'leavingSoonList'])->middleware('checkModule')->name('leavingsoon');
...
Route::post('/notifications/mark-all-read', [\Modules\Frontend\Http\Controllers\NotificationController::class, 'markAllAsRead'])->name('notifications.markAllAsRead');
...
Route::post('/continue-watch/store', [DashboardController::class, 'storeContinueWatch'])
    ->name('frontend.continueWatch.store');
Route::post('/continue-watch/resume', [DashboardController::class, 'resumeContinueWatch'])
    ->name('frontend.continueWatch.resume');

Route::get('/test/rent-movie/{id}', function ($id) {
    if (!auth()->check()) return "Login first";
    ...
    $controller = app(\Modules\Frontend\Http\Controllers\PerviewPaymentController::class);
    return $controller->savePaymentPayperview($request);
})->middleware('auth');
